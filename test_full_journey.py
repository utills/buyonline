...
def run_lead_otp(page: Page, mobile: str) -> bool:
    ...
    def handle_response(response):
        if 'localhost:3001' in response.url and not response.ok:
    ...
    time.sleep(2)  # allow API round-trip (lead + OTP send)
    img2 = shot(page, "A_after_submit")  # capture post-submit state
    if api_errors:
        print(f"      API errors: {api_errors}")
    section("A2 · OTP Verification")
    if not wait_url(page, r"/otp-verify", ms=15_000):
        check("Navigated to /otp-verify", False, f"url={page.url} api_errors={api_errors}")
        return False
    check("Navigated to /otp-verify", True)

    try:
        page.wait_for_selector("input[maxlength='1']", timeout=8_000)
...
def run_payment(page: Page) -> bool:
    ...
    if ok:
        ...
        # Navigate to KYC
        click_btn(page, "Continue to KYC", "Continue", "Next"); time.sleep(1)
    return ok
...
def run_kyc(page: Page) -> bool:
    ...
    fill(page, "input[placeholder='ABCDE1234F']", "ABCDE1234F")
    fill(page, "input[type='date']", "1990-01-15")
    click_btn(page, "Verify PAN", "Verify", "Continue", "Submit"); time.sleep(1)
...
def run_health_declaration(page: Page) -> bool:
    ...
    # Title is a required select — pick first non-empty option
    try:
        page.locator("select").first.select_option(index=1)
    except Exception:
        pass
    fill(page, "input[placeholder='First']", "Test")
    fill(page, "input[placeholder='Last']",  "User")
    fill(page, "input[type='date']",          "1990-01-15")
    fill(page, "input[placeholder='ft']",     "5")
    fill(page, "input[placeholder='in']",     "8")
    fill(page, "input[placeholder='kg']",     "70")
    # "Save Details" saves each member form; then page-level "Continue" navigates
    click_btn(page, "Save Details"); time.sleep(0.8)
    click_btn(page, "Continue"); time.sleep(1)
    ...
    fill(page, "input[placeholder*='State Bank']",             "State Bank of India")
    fill(page, "input[placeholder*='SBIN']",                   "SBIN0001234")
    click_btn(page, "Continue"); time.sleep(1)
    ...
    if ok:
        check("Completion message visible", any(k in body for k in ["complete", "Complete", "Thank", "Submitted", "submitted", "success", "next", "policy"]))
    return ok
...
def suite_b_plan_toggle(ctx: BrowserContext) -> None:
    # Default tier is SIGNATURE → plan-signature = "PHI Flagship 1" in DB = "Signature" in configurator
    section("B1 · Configurator: Disable Signature plan (plan-signature → 'PHI Flagship 1')")
    page = ctx.new_page()
    ...
    if opened:
        ...
        # PlanConfigCard: "Signature" for plan-signature (shown as "PHI Flagship 1" in DB)
        try:
            page.wait_for_selector("text=Signature", timeout=5_000)
            check("Signature plan visible in configurator", True, img=img)
            toggle = page.locator('[aria-label="Disable Signature"]').first
            toggle.scroll_into_view_if_needed()
            toggle.click()
            time.sleep(0.5)
            check("Signature plan disabled (PHI Flagship 1 hidden)", True)
        except PWTimeout:
            check("Signature plan visible in configurator", False)
        except Exception as e:
            check("Signature plan disable toggle", False, str(e)[:80])

    page.close()

    # Verify "PHI Flagship 1" (plan-signature, default SIGNATURE tier) is absent
    section("B2 · Verify: PHI Flagship 1 hidden in Classic Journey Plans")
    p2 = ctx.new_page()
    mobile = new_mobile()
    ...
    if ok:
        ...
        if wait_url(p2, r"/plans", ms=10_000):
            ...
            img = shot(p2, "B_plans_no_flagship")
            flagship_gone = "PHI Flagship 1" not in body
            check("PHI Flagship 1 NOT shown in plans (config respected)", flagship_gone, img=img)
    p2.close()


def suite_b_addon_toggle(ctx: BrowserContext) -> None:
    section("B3 · Configurator: Disable Maternity Coverage addon")
    page = ctx.new_page()
    # Navigate directly to /configurator/plans (addons are at bottom of that page)
    page.goto(f"{WEB}/configurator/plans", wait_until="domcontentloaded")
    time.sleep(2)
    opened = "/configurator" in page.url
    check("Configurator plans page opened", opened, f"url={page.url}")

    # Plans page has a tab switcher: "Plans" (default) and "Add-ons"
    # Must click the "Add-ons" tab to reveal the addon list
    try:
        page.get_by_role("button", name=re.compile("Add-ons|Addons", re.IGNORECASE)).first.click()
        time.sleep(1)
        check("Add-ons tab clicked", True)
    except Exception as e:
        check("Add-ons tab clicked", False, str(e)[:60])

    # AddonConfigList aria-label: "Disable Maternity Coverage" when enabled
    try:
        page.wait_for_selector('[aria-label*="Maternity"]', timeout=8_000)
        page.evaluate('document.querySelector(\'[aria-label*="Maternity"]\').click()')
        time.sleep(0.5)
        check("Maternity Coverage addon disabled", True)
    except PWTimeout:
        check("Maternity Coverage addon disabled", False, "timeout waiting for [aria-label*='Maternity']")
    except Exception as e:
        check("Maternity Coverage addon disabled", False, str(e)[:80])
    page.close()

    section("B4 · Verify: Maternity Coverage hidden in Classic Journey Addons")
    p2 = ctx.new_page()
    mobile = new_mobile()
    ok = run_lead_otp(p2, mobile)
    if ok:
...
def suite_b_hospital_flag(ctx: BrowserContext) -> None:
    ...
    # FlagRow aria-label: "Disable Hospital Search" when enabled
    try:
        page.wait_for_selector('[aria-label="Disable Hospital Search"]', timeout=8_000)
        toggle = page.locator('[aria-label="Disable Hospital Search"]').first
        toggle.scroll_into_view_if_needed()
        toggle.click()
        time.sleep(0.5)
        ...
        try:
            # Try "Enable" variant in case it's already off
            toggle = page.locator('[aria-label*="Hospital Search"]').first
            toggle.click()
            time.sleep(0.5)
            check("Hospital Search flag toggled", True)
        except Exception as e:
    ...
    section("B6 · Verify: /hospitals shows disabled message when flag is OFF")
    p2 = ctx.new_page()
    p2.goto(f"{WEB}/hospitals")
    time.sleep(2)
    img = shot(p2, "B_hospitals_disabled")
    body = p2.inner_text("body")
    # Page shows "Hospital search is not available." message instead of the hospital list
    disabled_msg = "not available" in body.lower() or "hospital search" in body.lower()
    check("Hospital search disabled message shown (config respected)", disabled_msg, f"url={p2.url}", img=img)
    p2.close()
...
def suite_b_reset(ctx: BrowserContext) -> None:
    ...
    open_configurator(page)
    try:
        # ConfigHeader reset button: title="Reset all configuration to defaults"
        # First click shows "Click again to confirm", second click resets
        reset_btn = page.locator('[title="Reset all configuration to defaults"]').first
        reset_btn.wait_for(timeout=5_000)
        reset_btn.click()
        time.sleep(0.4)
        # Second click to confirm
        try:
            confirm = page.locator('button:has-text("Click again to confirm")').first
            confirm.click(timeout=2_000)
        except Exception:
            # Maybe it reset immediately on first click
            pass
        time.sleep(0.8)
        ...
        try:
            page.get_by_role("button", name=re.compile("Reset defaults", re.IGNORECASE)).first.click()
            time.sleep(0.4)
            try:
                page.get_by_role("button", name=re.compile("Click again|Confirm", re.IGNORECASE)).first.click(timeout=2_000)
            except Exception:
                pass
            time.sleep(0.8)
            check("Config reset to defaults", True)
        except Exception as e2:
            check("Config reset button found", False, str(e2)[:60])
    page.close()

    section("B8 · Verify: PHI Flagship 1 reappears after reset")
    p2 = ctx.new_page()
    mobile = new_mobile()
    ok = run_lead_otp(p2, mobile)
    if ok:
        ...
        if wait_url(p2, r"/plans", ms=10_000):
            ...
            check("PHI Flagship 1 reappears after config reset", "PHI Flagship 1" in body, img=img)
    p2.close()
